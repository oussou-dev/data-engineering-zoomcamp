...
BigQuery automatically records the information 'how many rows is there in this table' in a separate 'metadata store' every time data comes into a table, so there is no need to scan the data. As a result, the estimate is 0 bytes.
